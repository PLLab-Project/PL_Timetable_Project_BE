...
class ScheduleSearchServiceTest {
    ...
    void search_returnsCombinationsWithinCreditRangeContainingAllRequiredCourses() {
        ...
        for (ScheduleCombination combination : results) {
            assertThat(combination.totalCreditUnits()).isBetween(900, 1400);
            assertThat(combination.courses())
    ...
    void search_neverSelectsConflictingCandidatesTogether() {
        ...
        assertThat(results).isNotEmpty();
        for (ScheduleCombination combination : results) {
            Set<SectionReference> sections = sections(combination);
            assertThat(sections.containsAll(Set.of(
                    new SectionReference("2026-1", "C20", "01"),
                    new SectionReference("2026-1", "C21", "01")))).isFalse();
        }
    }
    ...
    void search_findsThreeDistinctTopCombinationsWhenManyTiedOptimaExist() {
        ...
        Set<Set<SectionReference>> distinctCombinations = new HashSet<>();
        for (ScheduleCombination combination : results) {
            assertThat(combination.totalCreditUnits()).isEqualTo(600);
            assertNoConflicts(combination.courses());
            distinctCombinations.add(sections(combination));
        }
    ...
    private Set<SectionReference> sections(ScheduleCombination combination) {
        return combination.courses().stream()
    ...
    private OptimizationConstraints constraints(int minCredit, int maxCredit, int targetCredit, long searchTimeLimitMillis) {
        return new OptimizationConstraints(
                minCredit * 100, maxCredit * 100, targetCredit * 100, Set.of(), Set.of(),
