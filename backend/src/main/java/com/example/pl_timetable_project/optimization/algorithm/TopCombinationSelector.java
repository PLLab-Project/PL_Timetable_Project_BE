...
public class TopCombinationSelector {
    ...
    public double overlapRatio(List<CandidateCourse> a, List<CandidateCourse> b) {
        Set<SectionReference> sectionsA = sections(a);
        Set<SectionReference> sectionsB = sections(b);

        Set<SectionReference> intersection = new HashSet<>(sectionsA);
        intersection.retainAll(sectionsB);

        Set<SectionReference> union = new HashSet<>(sectionsA);
        union.addAll(sectionsB);
    ...
    private Set<SectionReference> sections(List<CandidateCourse> courses) {
        return courses.stream().map(CandidateCourse::section).collect(Collectors.toSet());
    }
}
