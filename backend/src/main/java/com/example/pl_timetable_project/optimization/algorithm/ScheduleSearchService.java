...
public class ScheduleSearchService {
    ...
    private ModelContext buildModel(List<CandidateCourse> requiredCourses, List<CandidateCourse> optionalCandidates,
        ...
        long[] credits = optionalCandidates.stream().mapToLong(CandidateCourse::credit).toArray();
        if (vars.length > 0) {
            creditBuilder.addWeightedSum(vars, credits);
        }
        LinearExpr totalCreditExpr = creditBuilder.build();
        model.addLinearConstraint(totalCreditExpr, constraints.minCredit(), constraints.maxCredit());
        ...
        // ---- 목표학점과의 차이(절댓값) ----
        int totalOptionalCredit = optionalCandidates.stream().mapToInt(CandidateCourse::credit).sum();
        int creditDiffBound = requiredCredit + totalOptionalCredit + Math.abs(constraints.targetCredit()) + 1;
        IntVar absCreditDiff = model.newIntVar(0, creditDiffBound, "absCreditDiff");
        LinearExpr creditDiffExpr = LinearExpr.affine(totalCreditExpr, 1, -constraints.targetCredit());
