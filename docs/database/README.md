...
페이로드를 임시 디렉터리에서 자동으로 풀고 검증한 뒤에만 기존 파일을 교체합니다.

```bash
./start.sh
```

백엔드 API 실행 없이 로컬 DB만 준비하는 개발자는 다음 명령을 사용합니다.

```bash
./load-db.sh
```

`load-db.sh`는 번들 검증·압축 해제 후 로컬 Compose의 `db`만 시작하고, 일회성
`migrate`와 `ingest` 컨테이너를 순서대로 실행합니다. `api` 이미지를 빌드하거나
컨테이너를 시작·재시작하지 않으며 Cloud SQL이나 다른 외부 DB에도 접근하지 않습니다.
같은 데이터 패키지를 다시 실행하면 멱등 검증만 수행해 실행 중 생성 데이터를
유지합니다. 다만 새로운 카탈로그 패키지를 기존 로컬 DB에 처음 적용하면 학사 테이블을
재구성하면서 이에 종속된 `completed_courses`, 시간표·자동편성 개발 데이터가 정리될 수
있으므로, 빈 개발 DB에서 실행하거나 필요한 로컬 데이터를 먼저 백업합니다.
