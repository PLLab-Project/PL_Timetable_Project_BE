...
API를 빌드하거나 실행하지 않고 로컬 PostgreSQL의 스키마와 학사 데이터만 준비하려면
다음 DB 전용 스크립트를 사용합니다.

```bash
./load-db.sh
```

이 명령은 `db`, 일회성 `migrate`, 일회성 `ingest` 서비스만 사용합니다. API 컨테이너,
프론트엔드, Cloud SQL에는 접근하지 않습니다. 기존 데이터가 있는 로컬 DB에 새로운
학사 데이터 패키지를 처음 적용할 때는 학사 카탈로그와 이에 종속된 로컬 개발 데이터가
재구성될 수 있으므로 필요한 데이터는 먼저 백업합니다.
